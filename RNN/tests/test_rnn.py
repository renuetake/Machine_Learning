...
class test_rnn(unittest.TestCase):
    def test_init_net(self):
        ...
        with self.assertRaises(ValueError):
            ...
            normal_input = 10
            rnn.init_net(error_input, error_input, error_input)
        with self.assertRaises(ValueError):
            error_input = 'test_input'
            normal_input = 10
            rnn.init_net(normal_input, error_input, error_input)
        with self.assertRaises(ValueError):
            error_input = 'test_input'
            normal_input = 10
            rnn.init_net(error_input, normal_input, error_input)
        with self.assertRaises(ValueError):
            error_input = 'test_input'
            normal_input = 10
            rnn.init_net(error_input, error_input, normal_input)
        with self.assertRaises(ValueError):
            error_input = 'test_input'
            normal_input = 10
            rnn.init_net(normal_input, normal_input, error_input)
        with self.assertRaises(ValueError):
            error_input = 'test_input'
            normal_input = 10
            rnn.init_net(error_input, normal_input, normal_input)
        with self.assertRaises(ValueError):
            error_input = 'test_input'
            normal_input = 10
            rnn.init_net(normal_input, error_input, normal_input)
        ...
        actual = type(rnn.init_net(num_input, num_hidden, num_output))
        ...
        return_list = rnn.init_net(num_input, num_hidden, num_output)
        ...
        return_list = rnn.init_net(num_input, num_hidden, num_output, seed=10)
        # 重み1(入力層~隠れ層)のテスト
    ...
    def test_feedforward(self):
        ...
        with self.assertRaises(ValueError):
            ...
            rnn.feedforward(error_input_str, error_input_int, error_input_int)
        ...
        actual = type(rnn.feedforward(weight, data, isample))
        self.assertEqual(expected, actual)
        ...
        actual = rnn.feedforward(weight, data, isample)
        self.assertEqual(expected, actual)
    ...
    def test_backward(self):
        ...
        with self.assertRaises(ValueError):
            error_input_int = 10
            error_input_str = 'test'
            rnn.backward(error_input_str, error_input_int, error_input_int, error_input_str)
        ...
        expected = type([[1,2],[1,2]])
        actual = type(rnn.backward(weight, data, isample, out))
        self.assertEqual(expected, actual)
        ...
        actual = rnn.backward(weight, data, isample, out)
        self.assertEqual(expected, actual)
