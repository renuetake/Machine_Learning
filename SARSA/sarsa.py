...
def update_q(q_table, agent, reward, alpha, gamma):
    ...
    next_q = q_table[next_x, next_y, next_action] # 次時刻の位置で次時刻の行動をとった時のQ値
    ret_q = now_q + alpha * (reward + (gamma * next_q) - now_q)
    return ret_q
...
def plot_result(row, col, log):
    ...
    # pngファイルに書き出し
    plt.savefig(DIR+'/'+'Trajectory_of_Agent-SARSA.png')

    # 標準出力に最大のQ値を取る行動をプリント
    q_table = np.load(DIR+'/SARSA_table.npy')
    for i in range(11):
        for j in range(19):
...
if __name__ == '__main__':
    ...
    # 学習済みのQ値を外部ファイルに書き出し
    np.save(DIR+'/SARSA_table.npy', q_table)
